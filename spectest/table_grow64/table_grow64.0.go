// Code generated by wasm2go. DO NOT EDIT.

package wasm2go

type Module struct {
	t0 []any
}

func New() *Module {
	m := &Module{}
	return m
}
func (m *Module) Xget응2Dt64(v0 int64) any {
	t0 := m.t0[v0]
	return t0
}
func (m *Module) Xset응2Dt64(v0 int64, v1 any) {
	m.t0[v0] = v1
}
func (m *Module) Xgrow응2Dt64(v0 int64, v1 any) int64 {
	t0 := table_grow(&m.t0, v1, v0, 281474976710656)
	return t0
}
func (m *Module) Xsize응2Dt64() int64 {
	t0 := int64(len(m.t0))
	return t0
}

func table_grow[T int32 | int64](tab *[]any, val any, delta, max T) T {
	buf := *tab
	len := len(buf)
	old := T(len)
	if delta == 0 {
		return old
	}
	new := old + delta
	add := int(new) - len
	if new > max || add < 0 {
		return -1
	}
	buf = append(buf, make([]any, add)...)
	if val != nil {
		cpy := buf[len:]
		for i := range cpy {
			cpy[i] = val
		}
	}
	*tab = buf
	return old
}
