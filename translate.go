...
type translator struct {
	...
	data          []dataSegment
	dylink        *dylinkDef
	codeStart     uint64            // start offset of code section content
	debugSections map[string][]byte // .debug_* custom sections for DWARF
}
...
const (
	...
	dwarfPCPfx   = "_dwarfPC_"   // prefix for per-instruction offset markers
)
...
func injectDwarfLines(buf []byte, sections map[string][]byte) ([]byte, error) {
	...
	for raw := range bytes.Lines(buf) {
		...
		case trimmed == dwarfPCEnd:
			...
			cur = nil
			continue
		case inCode && strings.HasPrefix(trimmed, dwarfPCPfx):
			offset, err := strconv.ParseUint(trimmed[len(dwarfPCPfx):], 10, 64)
			if err != nil {
		...
		lineNo++
		if !inCode {
			cur = &dwarfLine{file: "wasm2go.go", line: lineNo, col: 1}
		}
		...
		if cur != nil && !strings.HasPrefix(trimmed, "/*") && !strings.HasPrefix(trimmed, "//") {
			...
			out.WriteByte(':')
			// note: column numbers in stack traces will be misleading since go
			// increments it for columns in the generated code
